...
class DataValidation:
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        try:
            threshold = self.data_validation_config.missinng_threshold
            null_report = df.isna().sum()/df.shape[0]
            #selecting column name which contains null
            logging.info(f"selecting column names which contains null above {threshold}")
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            base_df = utils.convert_columns_float(df=base_df, exclude_columns=exclude_column)
            train_df = utils.convert_columns_float(df=train_df, exclude_columns=exclude_column)
            test_df = utils.convert_columns_float(df=test_df, exclude_columns=exclude_column)
